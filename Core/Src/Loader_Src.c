...
Init(void) {
    ...
    */
    ...
    if (CSP_USART_WriteMemoryFromInit((uint8_t*) NULL, (0 & (0x0fffffff)), 100) != HAL_OK) {
        __set_PRIMASK(1); //disable interrupts
        return LOADER_FAIL;
    }
...
Write(uint32_t Address, uint32_t Size, uint8_t* buffer) {
    ...
    if (CSP_USART_WriteMemory((uint8_t*) buffer, (Address & (0x0fffffff)), Size) != HAL_OK) {
        __set_PRIMASK(1); //disable interrupts
        return LOADER_FAIL;
    }
